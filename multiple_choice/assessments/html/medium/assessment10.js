    ...
    questions: [
        {
            ...
            correct: 'A',
            explanation:
                'When a user denies permission the error callback is invoked with a PositionError (PERMISSION_DENIED). The device failing to get a fix will also call the error callback with POSITION_UNAVAILABLE.',
            topics: ['geolocation']
        },

        {
            ...
            options: { A: 'sessionStorage', B: 'localStorage', C: 'IndexedDB only (Web Storage is temporary)', D: 'Neither; both are cleared when the browser closes' },
            correct: 'B',
...

        {
            ...
            options: { A: '<pre>spellcheck="false"</pre> (attribute) or element.spellcheck = false', B: '<pre>contenteditable="false"</pre> disables spellcheck', C: 'Add <pre>translate="no"</pre> to the element', D: 'Use CSS property <pre>user-select: none</pre>' },
            correct: 'A',
...

        {
            ...
            options: { A: 'That the content should not be machine-translated', B: 'That the content must be translated', C: 'Only affects spellcheck', D: 'Replaces lang attribute' },
            correct: 'A',
...

        {
            ...
            options: { A: 'localStorage still works because it is same-origin storage, independent of third-party cookie settings', B: 'localStorage is blocked whenever any cookie setting is disabled', C: 'localStorage becomes temporary and is cleared on tab close', D: 'localStorage writes silently fail and throw a SecurityError' },
            correct: 'A',
...

        {
            ...
            options: { A: 'It contains a set of key/value pairs representing data being transferred and is used on dragstart/drop handlers', B: 'It is only present on the drop target and not accessible from the drag source', C: 'It automatically serializes complex objects without developer intervention', D: 'It is a wrapper for navigator.clipboard during drag-and-drop' },
            correct: 'A',
...

        {
            ...
            options: { A: 'event.clipboardData.getData("text/html") inside paste handler', B: 'navigator.clipboard.readText() only provides text, not HTML', C: 'document.execCommand("getHTML")', D: 'There is no way to read HTML from the clipboard in the browser' },
            correct: 'A',
...

        {
            ...
            options: { A: 'enableHighAccuracy', B: 'maximumAge', C: 'timeout', D: 'frequency' },
            correct: 'B',
...

        {
            ...
            options: { A: 'Throw an exception', B: 'Do nothing (no error)', C: 'Clear all storage for the origin', D: 'Return false and leave storage untouched' },
            correct: 'B',
...

        {
            ...
            options: { A: 'Call event.preventDefault() in the dragover handler', B: 'Set the target element to contenteditable', C: 'Call event.stopPropagation()', D: 'Change the element\u2019s draggable attribute to false' },
            correct: 'A',
...

        {
            ...
            options: { A: 'document.activeElement === theElement (and check selection.rangeCount)', B: 'theElement.hasAttribute("focused")', C: 'theElement.contentEditable === "true" only', D: 'theElement.isContentSelected' },
            correct: 'A',
...

        {
            ...
            options: { A: 'Call navigator.geolocation.clearWatch(id) when updates are no longer needed', B: 'Nothing — watchPosition stops automatically when the page is hidden', C: 'Call stop() on the returned Watcher object', D: 'Call navigator.geolocation.cancelWatch()' },
            correct: 'A',
...

        {
            ...
            options: { A: 'Store raw HTML and sanitize on output with a robust sanitizer', B: 'Trust the paste HTML and store it directly', C: 'Replace all tags with their entity-escaped equivalents and show as text', D: 'Remove text nodes and store only element names' },
            correct: 'A',
...

        {
            ...
            options: { A: 'Storage quota exceeded for the origin (too much data stored)', B: 'The key name contains invalid characters', C: 'Because localStorage is read-only and never allows writes', D: 'Because the origin is in the browser’s allowlist' },
            correct: 'A',
...

        {
            ...
            options: { A: 'Also provide keyboard alternatives and ARIA attributes to indicate draggable and droppable roles', B: 'Only support mouse users; ARIA is unnecessary', C: 'Wrap draggable elements in <pre>button</pre> elements', D: 'Use contenteditable instead of drag-and-drop' },
            correct: 'A',
...

        {
            ...
            options: { A: '<pre>&lt;span translate="no"&gt;ID: 42&lt;/span&gt;</pre>', B: '<pre>&lt;span spellcheck="false"&gt;ID: 42&lt;/span&gt;</pre>', C: '<pre>&lt;span contenteditable="false"&gt;ID: 42&lt;/span&gt;</pre>', D: '<pre>&lt;span lang="en"&gt;ID: 42&lt;/span&gt;</pre>' },
            correct: 'A',
...

        {
            ...
            options: { A: 'Call event.preventDefault() in dragover and drop handlers, then use event.dataTransfer.files in drop', B: 'Only preventDefault in dragstart, then read files from navigator.files', C: 'Set draggable=false on the target to allow file reads', D: 'Use localStorage to store the File objects directly' },
            correct: 'A',
...

        {
            ...
            options: { A: 'localStorage stores values as strings, so you must parseInt/parseFloat when reading numbers back', B: 'Numbers are stored with higher precision in localStorage', C: 'A numeric key will be auto-converted to a number when read', D: 'localStorage preserves type information (no conversion needed)' },
            correct: 'A',
...

        {
            ...
            options: { A: 'dragstart → dragenter → dragover → drop → dragend', B: 'dragstart → drop → dragenter → dragover → dragend', C: 'dragstart → drag → drop → dragend → dragover', D: 'dragstart → dragend → drop' },
            correct: 'A',
...

        {
            ...
            options: { A: 'Store the position object (latitude/longitude/timestamp) in localStorage or IndexedDB and use it when offline', B: 'Rely on navigator.geolocation.getCurrentPosition even when offline', C: 'Store the Position object directly in sessionStorage without serialization', D: 'Write the position into a cookie with large expiry' },
            correct: 'A',
...

        {
            ...
            options: { A: 'role="option"', B: 'aria-grabbed (or set aria-dropeffect when appropriate) alongside draggable', C: 'translate="no"', D: 'contenteditable="true"' },
            correct: 'B',
...

        {
            ...
            options: { A: 'User gesture and secure context (HTTPS); some clipboard writes require explicit permission', B: 'No permission is ever required for clipboard writes', C: 'Clipboard writes only work in contenteditable elements', D: 'Clipboard API is available only in service workers' },
            correct: 'A',
